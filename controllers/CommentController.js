...
const CommentController = {
  ...
  async create(req, res, next) {
    ...
    try {
      ...
      if (!post) {
        res.status(400).send({ message: "The post was not created yet" });
      }
      ...
      res.status(200).send({ message: "You have commented", comment });
    } catch (error) {
      console.error(error);
      error.origin = "user";
      next(error);
  ...
  async update(req, res) {
    ...
    try {
      ...
      console.log(comment);
      if (!comment) {
        return res.status(404).send({ message: "The comment was not found" });
      }
      ...
      console.error(error);
      res.status(500).send({
        message: "There was a problem updating it",
